<div class="w-4/5 mx-auto my-8 bg-slate-500 shadow-sm shadow-red-900 border-[1px] border-yellow-700 py-2 px-3 relative">
    ...
    <div class="flex justify-end gap-4 my-2">
        <a href="/articlesCRUD/{{$article->id}}/edit" ><button class="bg-yellow-600 border-2 border-white shadow-sm shadow-black text-[1vw] p-2 rounded-md text-white font-semibold "> EDIT</button></a>
        <form action="/articlesCRUD/{{$article->id}}" method="post">
            @csrf
            @method('DELETE')
            
            <input type="submit" value="SUPPRIMER" class="bg-red-700 border-2 border-white shadow-sm shadow-black text-[1vw] p-2 rounded-md text-white font-semibold hover:cursor-pointer ">
        </form>
    </div>
</div>